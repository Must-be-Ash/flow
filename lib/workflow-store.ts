...
export type SkillNodeType = "purpose" | "service" | "instruction" | "decision" | "input" | "output";

// Legacy types kept for backward compat during migration
export type WorkflowNodeType = SkillNodeType | "trigger" | "action" | "add" | "x402" | "logic" | "data";
...
export type WorkflowNodeData = {
  ...
  notFor?: string;            // What this is NOT for

  // ── Service node fields ──
  endpoint?: EndpointData;    // x402 endpoint data
  notes?: string;             // User's notes about this service

  // ── Instruction node fields ──
  instruction?: string;       // Plain English instruction
  ...
  hints?: string;             // How to handle defaults
  required?: boolean;         // Is this input required?
  saveAs?: string;            // Where to persist (e.g. "data/recipient_<name>.md")

  // ── Output node fields ──
  format?: string;            // Output format ("Markdown report", etc.)
  template?: string;          // Output template content
...
// Per-node test state — persists across panel open/close
export type NodeTestState = {
  status: "idle" | "queued" | "working" | "done" | "error";
  stage?: string;           // "Checking schema...", "Paying $0.17...", "Generating image...", etc.
  chatJobId?: string;
  result?: unknown;         // the actual output (imageUrl, videoUrl, json, etc.)
  error?: string;
  startedAt?: string;
};
export const nodeTestStatesAtom = atom<Record<string, NodeTestState>>({});

// Increments whenever an external agent (MCP/chat) updates the workflow
// so the page knows to re-fetch from the server
export const workflowRefreshCountAtom = atom(0);
...
export const updateNodeDataAtom = atom(
  ...
  (get, set, { id, data }: { id: string; data: Partial<WorkflowNodeData> }) => {
    ...
    const newNodes = currentNodes.map((node) => {
      if (node.id === id) {
        // Update the node itself. If data.type changes, also update the
        // top-level node.type so React Flow picks the correct renderer.
        const merged = { ...node.data, ...data };
        const newType = data.type && data.type !== node.type ? data.type : node.type;
        return { ...node, type: newType, data: merged };
      }
...
// Load workflow by ID from file store
export const loadWorkflowAtom = atom(null, async (_get, set, workflowId: string) => {
  try {
